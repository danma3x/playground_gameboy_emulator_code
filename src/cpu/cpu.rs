...
macro_rules! g { // short for generate
    ($opcode:literal, $inst_size:literal, $timing:literal) => {
        |mut cpu, mut mmu, bytes|
        {
            ...
            println!("Running operation {}, op_block {}, op8m {} ", $opcode, op8, op8m);
            ...
            //
            expand!(OP_SRC_R_RA, { t_src = cpu.a as u64; println!("Loaded register A to t_src"); });
...
pub const INS_TABLE: [OpHandler; 64] = [
...
pub const CB_INS_TABLE: [OpHandler; 0] = [
...
mod tests {
    ...
    fn test_16bitld() {
        let ( mut cpu, mut mmu ) = prerequisites();
        INS_TABLE[0x01](&mut cpu, &mut mmu, [0x00, 0xFF, 0x9F, 0x00]);
        assert_eq!(cpu.b, 0x9f);
        assert_eq!(cpu.c, 0xff);
        INS_TABLE[0x11](&mut cpu, &mut mmu, [0x00, 0xFF, 0x9F, 0x00]);
        assert_eq!(cpu.d, 0x9f);
        assert_eq!(cpu.e, 0xff);
        INS_TABLE[0x21](&mut cpu, &mut mmu, [0x00, 0xFF, 0x9F, 0x00]);
        assert_eq!(cpu.h, 0x9f);
        assert_eq!(cpu.l, 0xff);
        INS_TABLE[0x31](&mut cpu, &mut mmu, [0x00, 0xFF, 0x9F, 0x00]);
        assert_eq!(cpu.sp, 0x9fff);
    }
    ...
    fn test_ld_8b_to_aa() {
        ...
        println!("{:?}", cpu);
        INS_TABLE[0x02](&mut cpu, &mut mmu, [0x0, 0x0, 0x0, 0x0]);
        assert_eq!(mmu.read_byte(0x100), 0xf9);
    }
    ...
    fn test_inc16() {
        let ( mut cpu, mut mmu ) = prerequisites();
        cpu.b = 0x01;
        INS_TABLE[0x03](&mut cpu, &mut mmu, [0x0, 0x0, 0x0, 0x0]);
        assert_eq!(cpu.b, 0x01);
        assert_eq!(cpu.c, 0x01);
    }

    // test half-carry and zero flag
    #[test]
    fn test_inc8() {
        let ( mut cpu, mut mmu ) = prerequisites();
        cpu.d = 50;
        INS_TABLE[0x14](&mut cpu, &mut mmu, [0x0, 0x0, 0x0, 0x0]);
        assert_eq!(cpu.d, 51);
    }
    ...
    fn test_inc8_to_a() {
        ...
        cpu.h = 0x1;
        cpu.l = 0x0;
        mmu.write_byte(0x100, 50);
        INS_TABLE[0x34](&mut cpu, &mut mmu, [0x0, 0x0, 0x0, 0x0]);
        assert_eq!(mmu.read_byte(0x100), 51);
    }
